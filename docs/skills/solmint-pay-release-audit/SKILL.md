...
At the time this skill was created, the live Supabase schema contained the Pay domain tables and RLS was enabled on them, but the database inspection reported **zero RLS policies on the Pay tables**. This is a critical authorization blocker, not a frontend defect.
...
The live security advisor also reported `SECURITY DEFINER` functions callable by `anon`/`authenticated` for existing site-media RPCs and reported `pg_net` in `public`. These findings are not automatically Pay blockers, but they must be triaged separately and must not be silently changed by Pay frontend work.
