...
function safeImage(value: unknown) {
  ...
  return /^(https?:|\/)/i.test(raw) && !/^(javascript|data|vbscript):/i.test(raw)
    ? raw
    : `${SITE_ORIGIN}/images/blog-og.jpg`;
}
...
export async function onRequest(context: PageContext): Promise<Response> {
  ...
  if (key) {
    try {
      ...
      if (response.ok) {
        ...
        articles = Array.isArray(rows)
          ? rows.filter((row: ArticleRecord) => Boolean(row.slug) && !row.is_draft)
          : [];
      }
    } catch {
  ...
  headers.set('X-Solmint-SSR', 'blog-discovery-v2');
  headers.set('X-Robots-Tag', 'index, follow');

  return new Response(inject(html, articles), {
