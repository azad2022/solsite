...
export const onRequest = async (context: { request: Request; env?: Env; next: () => Promise<Response> }): Promise<Response> => {
  ...
  if (!key) return new Response('Category SEO configuration error', { status: 500, headers: { 'Content-Type': 'text/plain; charset=utf-8' } });
  ...
  try {
    ...
    if (!categoryId) return new Response('Category not found', { status: 404, headers: { 'Content-Type': 'text/plain; charset=utf-8', 'X-Robots-Tag': 'noindex' } });
    ...
    headers.set('X-Robots-Tag', articles.length >= 2 ? 'index, follow' : 'noindex, follow');
    headers.set('X-Solmint-SEO', `taxonomy-ssr-v1 category=${rawSlug}`);
    ...
    html = setMeta(html, 'robots', articles.length >= 2 ? 'index, follow, max-snippet:-1, max-image-preview:large, max-video-preview:-1' : 'noindex, follow');
    ...
    return new Response('Category temporarily unavailable', { status: 503, headers: { 'Content-Type': 'text/plain; charset=utf-8', 'X-Robots-Tag': 'noindex' } });
  }
};
