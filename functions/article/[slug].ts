...
function inject(html: string, article: ArticleRecord, related: RelatedArticle[]) {
  ...
  const jsonLd = { '@context': 'https://schema.org', '@type': 'BlogPosting', headline: article.title, description, url: canonical, mainEntityOfPage: { '@type': 'WebPage', '@id': canonical }, datePublished: published, dateModified: modified, inLanguage: locale.schemaLanguage, author: { '@type': 'Person', name: article.author?.name || 'تیم تحریریه سولمینت' }, publisher: { '@type': 'Organization', name: 'Solmint', url: `${SITE_ORIGIN}/`, logo: { '@type': 'ImageObject', url: `${SITE_ORIGIN}/og-solmint.png` } }, image: [image], articleSection: article.category || 'اخبار و تحلیل', keywords: article.tags || [], breadcrumb: { '@type': 'BreadcrumbList', itemListElement: breadcrumb } };
  const tags = Array.isArray(article.tags) && article.tags.length ? `<footer><h2>${locale.lang === 'en' ? 'Tags' : 'برچسب‌ها'}</h2><ul>${article.tags.map(tag => `<li><a href=\"${escapeHtml(tagUrl(String(tag)))}\">${escapeHtml(tag)}</a></li>`).join('')}</ul></footer>` : '';
  const shell = `<main id=\"article-ssr\" dir=\"${locale.dir}\" lang=\"${locale.lang}\"><article><header><nav aria-label=\"مسیر صفحه\"><a href=\"/\">سولمینت</a> / <a href=\"/blog\">وبلاگ</a> / ${category} / <span aria-current=\"page\">${escapeHtml(article.title)}</span></nav><h1>${escapeHtml(article.title)}</h1><p>${escapeHtml(description)}</p><p><time datetime=\"${escapeHtml(published)}\">${escapeHtml(article.published_at_jalali || article.published_at_gregorian || published)}</time> · ${escapeHtml(article.read_time_minutes || 5)} ${locale.lang === 'en' ? 'minutes' : 'دقیقه مطالعه'}</p></header>${image !== '#' ? `<figure><img src=\"${escapeHtml(image)}\" alt=\"${escapeHtml(article.title)}\" fetchpriority=\"high\"><figcaption>${escapeHtml(article.title)}</figcaption></figure>` : ''}<section aria-label=\"${locale.lang === 'en' ? 'Article body' : 'متن مقاله'}\">${renderBody(article.content || '')}</section>${tags}${relatedHtml(related)}</article></main>`;
  let result = html; result = setDocumentLanguage(result, locale.lang, locale.dir); result = setTitle(result, title); result = setCanonical(result, canonical); result = setMeta(result, 'description', description); result = setMeta(result, 'robots', 'index, follow, max-snippet:-1, max-image-preview:large, max-video-preview:-1'); result = setMeta(result, 'og:type', 'article', true); result = setMeta(result, 'og:url', canonical, true); result = setMeta(result, 'og:title', title, true); result = setMeta(result, 'og:description', description, true); result = setMeta(result, 'og:image', image, true); result = setMeta(result, 'og:locale', locale.ogLocale, true); result = setJsonLd(result, jsonLd); return result.replace(/<div id=\"root\"><\/div>/i, `<div id=\"root\">${shell}</div>`);
}
