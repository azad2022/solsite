...
const CATEGORY_SLUGS: Record<string, string> = {
  ...
  'آموزش ساخت NFT': 'nft', 'کیف پول سولانا': 'solana-wallet', 'ترید': 'trading', 'پراپ تریدینگ': 'prop-trading'
};
...
const categorySlug = (name?: string | null) => {
  const value = String(name || '').trim();
  return CATEGORY_SLUGS[value] || value.toLocaleLowerCase('fa-IR').replace(/[\u200c\s]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
};
...
function renderBody(value: string) {
  ...
  for (const raw of lines) {
    ...
    if (heading) { const level = Math.min(6, Math.max(2, line.match(/^#+/)?.[0].length || 2)); out.push(`<h${level}>${escapeHtml(heading[1])}</h${level}>`); continue; }
...
function setMeta(html: string, name: string, value: string, property = false) {
  ...
  const tag = `<meta ${attr}="${attr === 'property' ? name : name}" content="${escapeHtml(value)}">`;
...
function relatedHtml(items: RelatedArticle[]) {
  ...
  return `<aside id="related-articles" aria-label="مقالات مرتبط"><h2>مقالات مرتبط</h2><ul>${items.map(item => `<li><a href="${escapeHtml(articleUrl(item.slug))}">${item.cover_image ? `<img src="${escapeHtml(safeUrl(item.cover_image))}" alt="${escapeHtml(item.title)}" loading="lazy" decoding="async">` : ''}<span><strong>${escapeHtml(item.title)}</strong>${item.summary ? `<small>${escapeHtml(stripHtml(item.summary))}</small>` : ''}</span></a></li>`).join('')}</ul></aside>`;
}
...
function inject(html: string, article: ArticleRecord, related: RelatedArticle[]) {
  ...
  const jsonLd = {
    '@context': 'https://schema.org', '@type': 'BlogPosting', headline: article.title, description, url: canonical,
    mainEntityOfPage: { '@type': 'WebPage', '@id': canonical }, datePublished: published, dateModified: modified,
    author: { '@type': 'Person', name: article.author?.name || 'تیم تحریریه سولمینت' },
    publisher: { '@type': 'Organization', name: 'Solmint', url: `${SITE_ORIGIN}/` },
    image: [image], articleSection: article.category || 'اخبار و تحلیل', keywords: article.tags || [],
    breadcrumb: { '@type': 'BreadcrumbList', itemListElement: breadcrumb }
  };
  const tags = Array.isArray(article.tags) && article.tags.length ? `<footer><h2>برچسب‌ها</h2><ul>${article.tags.map(tag => `<li><a href="/blog/tag/${encodeURIComponent(String(tag).trim().toLocaleLowerCase('fa-IR').replace(/\s+/g, '-'))}">${escapeHtml(tag)}</a></li>`).join('')}</ul></footer>` : '';
...
export async function onRequest(context: PageContext): Promise<Response> {
  ...
  try {
    ...
    if (!article) {
      ...
      const headers = new Headers(fallback.headers); headers.set('X-Robots-Tag', 'noindex, follow');
    ...
    headers.set('Content-Type', 'text/html; charset=UTF-8'); headers.set('X-Robots-Tag', 'index, follow'); headers.set('X-Solmint-SSR', 'article-seo-v1');
