...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }) => {
  try {
    ...
    const targetResponse = await fetch(
      `${base}/rest/v1/users?select=id,role& id=eq.${encodeURIComponent(userId)}&limit=1`.replace('?select=id,role& id=', '?select=id,role&id='),
      { headers }
    );
    ...
    if (target.role === 'superadmin' && actor.role !== 'superadmin') {
      return jsonResponse({ success: false, message: 'حذف حساب superadmin فقط توسط superadmin مجاز است.' }, 403);
    }
