...
function db(env: Env) {
  ...
  if (!key) throw new Error('SUPABASE_SECRET_KEY is not configured in Cloudflare.');
...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }) => {
  try {
    ...
    if (!user || (user.role !== 'admin' && user.role !== 'superadmin')) return jsonResponse({ success: false, message: 'نشست مدیر معتبر نیست. لطفاً دوباره وارد شوید.' }, 401);
    ...
    if (newAdminPasscode) {
      // Keep the existing password update behavior for now; the next successful login upgrades it to PBKDF2.
      const passwordResponse = await fetch(`${base}/rest/v1/users?username=eq.admin`, { method: 'PATCH', headers: { ...headers, Prefer: 'return=minimal' }, body: JSON.stringify({ password_hash: await (async () => { const d = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(newAdminPasscode)); return Array.from(new Uint8Array(d)).map(b => b.toString(16).padStart(2, '0')).join(''); })() }) });
      if (!passwordResponse.ok) throw new Error(await passwordResponse.text());
    }
