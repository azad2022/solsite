...
export const onRequest = async ({ request, env }: { request: Request; env: Env }) => {
  ...
  try {
    const categoryResponse = await fetch(`${base}/rest/v1/article_categories?id=eq.${encodeURIComponent(categoryId)}&select=id,name,default_media_asset_id,default_media_url,default_media_mode,default_media_interval_ms&limit=1`, { headers: { apikey: key, Authorization: `Bearer ${key}`, Accept: 'application/json' } });
