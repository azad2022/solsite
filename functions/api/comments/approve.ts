...
export const onRequestPost = async ({ request, env }: { request: Request; env: CommentsEnv }) => {
  try {
    ...
    if (!commentId || typeof body.approved !== 'boolean') return jsonResponse({ success: false, message: 'اطلاعات تغییر وضعیت نامعتبر است.' }, 400);

    const { base, headers } = getDb(env);
    const response = await fetch(`${base}/rest/v1/comments?id=eq.${encodeURIComponent(commentId)}`, {
