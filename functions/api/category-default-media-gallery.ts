...
const admin = async (request: Request, env: GalleryEnv) => {
  try {
    ...
    return !!user && user.isActive && (user.role === 'admin' || user.role === 'superadmin' || (Array.isArray(user.permissions) && user.permissions.map(String).includes('articles')));
  } catch { return false; }
};
...
const normalizeGithubAssetId = (value: string) => { const id = value.trim(); return /^github_[0-9a-f]{40}$/i.test(id) ? id.slice('github_'.length).toLowerCase() : null; };
...
const ensureMediaAsset = async (env: GalleryEnv, asset: { publicUrl: string; path: string; sha?: string | null; filename: string; owner: string; repository: string; branch: string }) => {
  ...
  const insertResponse = await db(env, 'media_assets', { method: 'POST', headers: { Prefer: 'resolution=ignore-duplicates,return=representation' }, body: JSON.stringify({ id, provider: 'github', github_owner: asset.owner, github_repository: asset.repository, branch: asset.branch, path: asset.path, filename: asset.filename, public_url: publicUrl, mime_type: `image/${(asset.filename.split('.').pop() || 'webp').toLowerCase()}`, file_size: 0, width: 0, height: 0, sha: asset.sha || null, original_filename: asset.filename, alt_text: '', title: asset.filename }) });
...
const resolveFromAuthenticatedMediaService = async (env: GalleryEnv, request: Request, githubSha: string) => {
  ...
  const response = await fetch(`${getSupabaseUrl(env)}/functions/v1/github-media`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-solmint-better-auth-session': sessionToken, 'x-solmint-auth-source': 'better-auth', 'x-media-gateway-version': '5' },
    body: JSON.stringify({ action: 'list' }),
  });
  const data = await response.json().catch(() => null) as any;
  if (!response.ok || !data?.success) throw new Error(response.status === 403 ? 'GITHUB_MEDIA_SERVICE_FORBIDDEN' : `GITHUB_MEDIA_SERVICE_${response.status || 502}`);
  const asset = Array.isArray(data.assets) ? data.assets.find((item: any) => String(item?.sha || '').toLowerCase() === githubSha) : null;
  if (!asset?.path || !asset?.publicUrl) throw new Error('MEDIA_ASSET_NOT_FOUND');
  return ensureMediaAsset(env, { publicUrl: String(asset.publicUrl), path: String(asset.path), sha: String(asset.sha || githubSha), filename: String(asset.filename || String(asset.path).split('/').pop() || asset.path), owner: String(asset.githubOwner || data?.config?.githubOwner || 'azad2022'), repository: String(asset.githubRepository || data?.config?.githubRepository || 'solsite'), branch: String(asset.branch || data?.config?.branch || 'main') });
};
...
const resolveMediaAssetIds = async (env: GalleryEnv, request: Request, mediaAssetIds: string[]) => {
  ...
  for (const id of mediaAssetIds) {
    ...
    if (exactResponse.ok && Array.isArray(exact) && exact[0]?.id) { resolved.push(String(exact[0].id)); continue; }
...
export const onRequest = async ({ request, env }: { request: Request; env: GalleryEnv }) => {
  if (!(await admin(request, env))) return json({ success: false, message: 'دسترسی مدیریت تصاویر دسته‌بندی مجاز نیست.' }, 403);
  try {
    ...
    const response = await db(env, 'rpc/set_category_default_media_gallery', { method: 'POST', body: JSON.stringify({ p_category_id: categoryId, p_media_asset_ids: resolvedMediaAssetIds, p_mode: mode, p_interval_ms: intervalMs }) });
    ...
    const message = internal === 'SUPABASE_SECRET_KEY_NOT_CONFIGURED' ? 'کلید امن دیتابیس در محیط production تنظیم نشده است.' : internal === 'MEDIA_ADMIN_SESSION_MISSING' ? 'نشست Better Auth در درخواست رسانه ارسال نشده است.' : internal === 'MEDIA_ASSET_NOT_FOUND' ? 'تصویر انتخاب‌شده در مخزن رسانه شناسایی نشد. کتابخانه تصاویر را تازه‌سازی کنید.' : internal === 'MEDIA_ASSET_REGISTRATION_FAILED' ? 'ثبت تصویر در کتابخانه رسانه ناموفق بود.' : internal === 'GITHUB_MEDIA_SERVICE_FORBIDDEN' ? 'سرویس امن کتابخانه تصاویر اجازه دسترسی به مخزن GitHub را ندارد.' : 'سرویس تصاویر دسته‌بندی در پردازش درخواست با خطا مواجه شد.';
