...
function parseAlgorithm(env: PayInternalJwtEnv): SupportedAlgorithm {
  ...
  if (!PAY_INTERNAL_JWT_ALGORITHMS.includes(value as SupportedAlgorithm)) {
    throw new Error('SUPABASE_INTERNAL_JWT_ALGORITHM must be explicitly configured as ES256 or RS256.');
  }
...
function assertSafeUserId(userId: string): string {
  ...
  if (!normalized || normalized.length > 256) throw new Error('Pay internal JWT user id is invalid.');
  if (/[^\x21-\x7e]/.test(normalized)) throw new Error('Pay internal JWT user id contains unsupported characters.');
  return normalized;
}
