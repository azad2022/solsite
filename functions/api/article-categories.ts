...
type Category = {
  id: string; name: string; slug: string; description?: string; seo_title?: string; seo_description?: string;
  parent_id?: string | null; sort_order?: number; is_active?: boolean; created_at?: string; updated_at?: string;
};
...
function envValue(env: Env, key: keyof Env, fallback = ''): string { return String(env[key] || fallback).trim(); }
function json(data: unknown, status = 200) { return new Response(JSON.stringify(data), { status, headers: { 'Content-Type': 'application/json; charset=UTF-8', 'Cache-Control': 'no-store' } }); }
function adminAuthorized(request: Request, env: Env): boolean {
  const expected = envValue(env, 'ADMIN_PASSCODE');
  const supplied = (request.headers.get('x-admin-passcode') || request.headers.get('authorization')?.replace(/^Bearer\s+/i, '') || '').trim();
  return Boolean(expected && supplied && supplied === expected);
}
function supabase(env: Env) {
  const url = envValue(env, 'SUPABASE_URL', envValue(env, 'VITE_SUPABASE_URL', fallbackUrl)).replace(/\/$/, '');
  const key = envValue(env, 'SUPABASE_SERVICE_ROLE_KEY', envValue(env, 'SUPABASE_ANON_KEY', envValue(env, 'VITE_SUPABASE_ANON_KEY', fallbackAnon)));
  return { url, key };
}
...
function cleanCategory(input: any): Category {
  return {
    id: String(input.id || `cat-${crypto.randomUUID()}`),
...
function validate(category: Category) {
  if (!category.name || category.name.length > 120) return 'نام دسته‌بندی باید بین ۱ تا ۱۲۰ کاراکتر باشد.';
  if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(category.slug) || category.slug.length > 160) return 'Slug فقط باید شامل حروف انگلیسی کوچک، عدد و خط تیره باشد.';
  if (category.parent_id === category.id) return 'دسته‌بندی نمی‌تواند والد خودش باشد.';
  return null;
}
...
export const onRequest: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    if (method === 'POST') {
      const category = cleanCategory(await request.json());
      const error = validate(category);
      if (error) return json({ success: false, message: error }, 400);
      const response = await db(env, 'article_categories', { method: 'POST', headers: { Prefer: 'return=representation' }, body: JSON.stringify(category) });
      const data = await response.json().catch(() => null);
      if (!response.ok) {
        const message = response.status === 409 ? 'این Slug قبلاً استفاده شده است.' : 'ایجاد دسته‌بندی در Supabase ناموفق بود.';
        return json({ success: false, message, details: data }, response.status);
      }
    ...
    if (method === 'DELETE') {
      const articleCheck = await db(env, `articles?category_id=eq.${encodeURIComponent(id)}&select=id&limit=1`);
      const linked = await articleCheck.json().catch(() => []);
      ...
      const childCheck = await db(env, `article_categories?parent_id=eq.${encodeURIComponent(id)}&select=id&limit=1`);
      const children = await childCheck.json().catch(() => []);
    ...
    return json({ success: false, message: error?.message || 'خطای غیرمنتظره در سرویس دسته‌بندی‌ها.' }, 500);
  }
};
