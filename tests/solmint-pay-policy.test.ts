...
test('gas sponsorship fails closed when credit is insufficient', () => {
  const decision = evaluateGasSponsorship({
    enabled: true,
    fundingModel: 'solmint_funded',
    availableAtomic: 49n,
    perPaymentLimitAtomic: 100n,
    dailyLimitAtomic: 1000n,
    spentTodayAtomic: 0n,
    estimatedNetworkFeeAtomic: 50n,
  });
  assert.deepEqual(decision, { allowed: false, reason: 'NO_CREDIT' });
});

test('gas sponsorship obeys per-payment and daily limits', () => {
  const paymentLimit = evaluateGasSponsorship({
    enabled: true,
    fundingModel: 'merchant_funded',
    availableAtomic: 1000n,
    perPaymentLimitAtomic: 40n,
    dailyLimitAtomic: 1000n,
    spentTodayAtomic: 0n,
    estimatedNetworkFeeAtomic: 50n,
  });
  ...
  const dailyLimit = evaluateGasSponsorship({
    enabled: true,
    fundingModel: 'merchant_funded',
    availableAtomic: 1000n,
    perPaymentLimitAtomic: 100n,
    dailyLimitAtomic: 1000n,
    spentTodayAtomic: 980n,
    estimatedNetworkFeeAtomic: 50n,
  });
...
test('revenue recognition keeps referral liability inside gross gateway revenue', () => {
  assert.deepEqual(recognizeGatewayRevenue('1000', 2000), {
    grossGatewayFeeAtomic: '1000',
    referralCommissionAtomic: '200',
    netGatewayRevenueAtomic: '800',
  });
});
  ...
  const result = verifyPaymentTransaction(
    {
      ...
      merchantSettlementAtomic: '100000000',
    ...
    },
    {
      ...
      transfers: [
        { role: 'merchant_settlement', source: 'CUSTOMER_A', destination: 'MERCHANT', asset: 'USDC', tokenMint: 'USDC_MINT', amountAtomic: '100000000', instructionIndex: 0 },
        { role: 'gateway_fee', source: 'CUSTOMER_B', destination: 'SOLMINT', asset: 'USDC', tokenMint: 'USDC_MINT', amountAtomic: '1000000', instructionIndex: 1 },
      ],
    },
  );
