...
test('wallet verification enforces origin and authenticated merchant ownership before challenge access', () => {
  assert.match(routeSource, /originAllowed\(request, env\)/);
  assert.match(routeSource, /getAuthenticatedUser\(env, request\)/);
  assert.match(routeSource, /merchant\.owner_user_id !== user\.id/);
});
