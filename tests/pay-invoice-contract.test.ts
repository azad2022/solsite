...
test('Invoice GET and POST use the pay_invoices contract', () => {
  ...
  for (const field of [
    ...
    'asset', 'fee_payer', 'checkout_locale', 'due_at', 'status', 'created_at', 'updated_at',
  ]) {
...
test('Invoice financial input and reads remain integer atomic data', () => {
  ...
  assert.match(service, /amount_atomic/);
});