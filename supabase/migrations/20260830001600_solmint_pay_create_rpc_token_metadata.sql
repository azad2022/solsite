-- Keep the server-only Payment Intent RPC in lock-step with the payment domain.
-- The previous signature is retired rather than overloaded so callers cannot
-- accidentally select a legacy contract without token-program metadata.
...
begin
  if p_merchant_id is null or p_amount_atomic <= 0 or p_fee_bps <> 100
...
comment on function public.pay_create_payment_intent(
  uuid,text,numeric,text,text,text,integer,text,text,integer,text,numeric,numeric,numeric,text,text,timestamptz,jsonb,text,text,text
) is 'Server-only atomic Payment Intent creation with token metadata and idempotency.';
