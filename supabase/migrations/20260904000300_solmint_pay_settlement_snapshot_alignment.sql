-- Align Payment Intent creation with the canonical merchant settlement snapshot.
-- merchant_settlement_atomic is the authoritative merchant settlement leg and
-- must be derived from the same server-side accounting calculation as
-- merchant_net_atomic; callers must never be able to choose it independently.
...
begin
  ...
  insert into public.pay_payment_intents (
    ...
    fee_atomic, customer_total_atomic, merchant_net_atomic, merchant_settlement_atomic,
    ...
    v_expected_fee, v_expected_customer_total, v_expected_merchant_net, v_expected_merchant_net,
...
comment on function public.pay_create_payment_intent(
  uuid,text,numeric,text,text,text,integer,text,text,integer,text,numeric,numeric,numeric,text,text,timestamptz,jsonb,text,text,text
) is 'Server-only atomic Payment Intent creation; merchant settlement is derived from the canonical merchant-net snapshot and stored as the authoritative settlement leg.';
