-- Defense-in-depth for authoritative reconciliation.
-- The TypeScript verifier requires feePayer == sourceAuthority when no sponsor
-- is configured. The database must enforce the same invariant before financial
-- writes so a caller cannot bypass the verifier's payer binding.
-- Sponsored payments remain unsupported until an immutable sponsor address is
-- stored on the Payment Intent and validated here.
...
is 'Server-only atomic Pay reconciliation; DB enforces payer binding and rejects sponsored payments until an immutable sponsor address exists.';
