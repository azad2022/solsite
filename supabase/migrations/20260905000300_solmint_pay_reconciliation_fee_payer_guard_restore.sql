-- Restore the authoritative reconciliation fee-payer and sponsorship guards.
-- The preceding token-destination migration intentionally changed only token-account
-- semantics; this migration restores the stronger payer binding that must remain
-- the final definition of the reconciliation RPC.
...
begin
  ...
  if v_payment.gas_sponsored is distinct from false then
  ...
  if p_signature is null or p_signature='' or p_success is distinct from true
     ...
     or p_commitment is distinct from v_payment.verification_commitment
     or p_fee_payer is null or p_fee_payer='' then
    return jsonb_build_object('ok',false,'reason','OBSERVATION_INVARIANT_FAILED');
  end if;

  if p_asset is distinct from v_payment.asset or p_recipient is distinct from v_payment.recipient
     or p_observed_amount_atomic is null or p_observed_amount_atomic <> v_payment.merchant_settlement_atomic then
    return jsonb_build_object('ok',false,'reason','PAYMENT_SNAPSHOT_MISMATCH');
  end if;

  if p_network_fee_lamports is null or p_network_fee_lamports < 0 then
    return jsonb_build_object('ok',false,'reason','INVALID_NETWORK_FEE');
  end if;
  ...
  if p_asset='SOL' then
    if exists (
      ...
      where x->>'role' in ('merchant_settlement','gateway_fee')
        and (x->>'destination' is null or x->>'asset' is null or x->>'amountAtomic' is null
             or x->>'sourceAuthority' is null or x->>'destinationAuthority' is null)
    ) then return jsonb_build_object('ok',false,'reason','TRANSFER_INVARIANT_FAILED'); end if;
  else
    if exists (
      select 1 from jsonb_array_elements(p_transfers) x
      where x->>'role' in ('merchant_settlement','gateway_fee')
        and (x->>'destination' is null or x->>'destinationAuthority' is null or x->>'asset' is null
             or x->>'amountAtomic' is null or x->>'sourceAuthority' is null
             or x->>'tokenMint' is null or x->>'tokenProgram' is null or x->>'tokenDecimals' is null)
    ) then return jsonb_build_object('ok',false,'reason','TOKEN_ACCOUNT_INVARIANT_FAILED'); end if;
  end if;
  ...
  select count(*) into v_merchant_count
   ...
   where x->>'role'='merchant_settlement'
     ...
     and (
       (v_payment.asset='SOL' and x->>'destination'=v_payment.recipient and coalesce(x->>'tokenMint','')='' and coalesce(x->>'tokenProgram','')='' and coalesce(x->>'tokenDecimals','')='')
       or
       (v_payment.asset<>'SOL' and x->>'destinationAuthority'=v_payment.recipient and x->>'tokenMint'=v_payment.token_mint and x->>'tokenProgram'=v_payment.token_program and x->>'tokenDecimals'=v_payment.token_decimals::text)
     );
  ...
  select count(*) into v_fee_count
   ...
   where x->>'role'='gateway_fee'
     ...
     and (
       (v_payment.asset='SOL' and x->>'destination'=v_payment.fee_recipient and coalesce(x->>'tokenMint','')='' and coalesce(x->>'tokenProgram','')='' and coalesce(x->>'tokenDecimals','')='')
       or
       (v_payment.asset<>'SOL' and x->>'destinationAuthority'=v_payment.fee_recipient and x->>'tokenMint'=v_payment.token_mint and x->>'tokenProgram'=v_payment.token_program and x->>'tokenDecimals'=v_payment.token_decimals::text)
     );
  ...
  select x->>'sourceAuthority',x->>'destinationAuthority' into v_merchant_source_authority,v_merchant_destination_authority
    from jsonb_array_elements(p_transfers) x where x->>'role'='merchant_settlement';
  select x->>'sourceAuthority',x->>'destinationAuthority' into v_fee_source_authority,v_fee_destination_authority
    from jsonb_array_elements(p_transfers) x where x->>'role'='gateway_fee';
  ...
  if p_fee_payer<>v_merchant_source_authority then
  ...
  if v_merchant_destination_authority<>v_payment.recipient or v_fee_destination_authority<>v_payment.fee_recipient then
    return jsonb_build_object('ok',false,'reason',case when p_asset='SOL' then 'DESTINATION_INVARIANT_FAILED' else 'TOKEN_ACCOUNT_INVARIANT_FAILED' end);
  end if;

  select * into v_existing_transaction from public.pay_payment_transactions where signature=p_signature for update;
  if v_existing_transaction.id is not null then
    ...
    update public.pay_payment_transactions
       set slot=p_slot,block_time=p_block_time,observed_amount_atomic=p_observed_amount_atomic,
           asset=p_asset,recipient=p_recipient,reference_matched=p_reference_matched,confirmed=true,
           success=p_success,commitment=p_commitment,fee_payer=p_fee_payer,network_fee_lamports=p_network_fee_lamports,
           verification_status='verified',verified_at=p_verified_at,is_authoritative=true,
           raw_transaction=coalesce(p_raw_observation,'{}'::jsonb),observed_at=now(),rejection_reason=null
     where id=v_existing_transaction.id and is_authoritative=false
     returning id into v_transaction_id;
  else
    insert into public.pay_payment_transactions(
      payment_id,signature,slot,block_time,observed_amount_atomic,asset,recipient,reference_matched,
      confirmed,success,commitment,fee_payer,network_fee_lamports,verification_status,verified_at,
      is_authoritative,raw_transaction,observed_at,rejection_reason
    ) values (
      p_payment_id,p_signature,p_slot,p_block_time,p_observed_amount_atomic,p_asset,p_recipient,p_reference_matched,
      true,p_success,p_commitment,p_fee_payer,p_network_fee_lamports,'verified',p_verified_at,true,
      coalesce(p_raw_observation,'{}'::jsonb),now(),null
    ) returning id into v_transaction_id;
  end if;

  if v_transaction_id is null then return jsonb_build_object('ok',false,'reason','SIGNATURE_ALREADY_BOUND'); end if;

  insert into public.pay_payment_transfers(
    payment_transaction_id,transfer_role,source,destination,asset,amount_atomic,token_mint,token_program,
    token_decimals,source_authority,destination_authority,instruction_index
  )
  select v_transaction_id,x->>'role',nullif(x->>'source',''),x->>'destination',x->>'asset',(x->>'amountAtomic')::numeric,
         nullif(x->>'tokenMint',''),nullif(x->>'tokenProgram',''),
         case when x->>'tokenDecimals' is null or x->>'tokenDecimals'='' then null else (x->>'tokenDecimals')::integer end,
         nullif(x->>'sourceAuthority',''),nullif(x->>'destinationAuthority',''),
  ...
  values(p_payment_id,v_payment.asset,v_gross_fee,0,v_gross_fee,'eligible',p_verified_at) on conflict(payment_id) do nothing;
