-- An authoritative verified observation is final financial evidence for a payment.
-- A later rejected scan must never overwrite or relabel that record.
...
begin
  ...
  select id, payment_id, is_authoritative
    into v_id, v_payment_id, v_authoritative
  ...
  if v_id is not null and v_authoritative is true then
    return jsonb_build_object('ok', false, 'reason', 'AUTHORITATIVE_OBSERVATION_EXISTS');
  end if;
  ...
  if v_id is null then
    ...
    update public.pay_payment_transactions
     ...
     where id = v_id and payment_id = p_payment_id and is_authoritative = false;
  end if;
