...
declare
  v_key public.pay_api_keys%rowtype; v_new_key public.pay_api_keys%rowtype; v_existing_id uuid; v_existing_status text; v_existing_hash text; v_existing_body jsonb; v_body jsonb;
begin
  ...
  if v_existing_id is null then
    insert into public.pay_idempotency_keys(merchant_id,scope,idempotency_key,request_hash,status,resource_type,resource_id) values(p_merchant_id,'api-keys:rotate',p_idempotency_key,p_request_hash,'processing','api_key',v_key.id) returning id into v_existing_id;
  end if;
