-- Recompute all payment fee invariants inside the server-only Payment Intent RPC.
-- Caller-supplied monetary totals remain part of the contract for request-hash
-- compatibility, but they can never define the stored financial values.
...
begin
  ...
  -- Canonical 1% fee policy. The calculation deliberately happens inside the
  -- database security boundary so no trusted caller can alter the accounting
  -- amounts while keeping the same Payment Intent contract.
  ...
  return jsonb_build_object('state', 'created', 'response_status', 201,
    'response_body', jsonb_build_object('data', jsonb_build_object(
      'id', v_payment.id,
      'status', v_payment.status,
      'amountAtomic', v_payment.amount_atomic::text,
...
comment on function public.pay_create_payment_intent(
  uuid,text,numeric,text,text,text,integer,text,text,integer,text,numeric,numeric,numeric,text,text,timestamptz,jsonb,text,text,text
) is 'Server-only atomic Payment Intent creation; fee, customer total, and merchant net are recomputed and verified inside the database.';
