...
begin
  ...
  if not exists (
    ...
    return jsonb_build_object('authorized', false, 'code', 'FORBIDDEN');
  end if;
  ...
  from (
    select *
    from public.pay_webhooks
    where merchant_id = p_merchant_id
      and (p_webhook_id is null or id = p_webhook_id)
...
revoke all on function public.pay_read_webhooks(uuid, uuid, integer) from public;
