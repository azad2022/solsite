-- Explicit NULL hardening for the server-only Payment Intent RPC.
-- PostgreSQL comparisons with NULL evaluate to UNKNOWN; sensitive validation
-- must therefore reject NULL explicitly rather than relying on OR predicates.
...
begin
  ...
  -- Canonical 1% fee policy. Recompute inside the database security boundary.
...
comment on function public.pay_create_payment_intent(
  uuid,text,numeric,text,text,text,integer,text,text,integer,text,numeric,numeric,numeric,text,text,timestamptz,jsonb,text,text,text
) is 'Server-only atomic Payment Intent creation; fee, customer total, and merchant net are recomputed and verified inside the database with explicit NULL rejection.';
