-- Token reconciliation must distinguish token-account destination from wallet owner.
-- For SPL/Token-2022 transfers, `destination` is the token account while
-- `destinationAuthority` is the wallet that owns it. Payment Intent recipient is
-- the merchant wallet, so authoritative matching must bind to the owner authority.
...
begin
  ...
  if exists (select 1 from jsonb_array_elements(p_transfers) x
             where x->>'role' in ('merchant_settlement','gateway_fee')
               and (x->>'destination' is null or x->>'asset' is null
                    or x->>'amountAtomic' is null or x->>'sourceAuthority' is null)) then
  ...
  insert into public.pay_payment_transactions(
    ...
    p_network_fee_lamports, 'verified', p_verified_at, true, coalesce(p_raw_observation,'{}'::jsonb), now(), null
  )
  on conflict (signature) do update set
    slot=excluded.slot, block_time=excluded.block_time, observed_amount_atomic=excluded.observed_amount_atomic,
    asset=excluded.asset, recipient=excluded.recipient, reference_matched=excluded.reference_matched,
    confirmed=true, success=excluded.success, commitment=excluded.commitment, fee_payer=excluded.fee_payer,
    network_fee_lamports=excluded.network_fee_lamports, verification_status='verified', verified_at=excluded.verified_at,
    is_authoritative=true, raw_transaction=excluded.raw_transaction, observed_at=now(), rejection_reason=null
  ...
  insert into public.pay_payment_transfers(
    ...
    token_mint, token_program, token_decimals, source_authority, destination_authority, instruction_index
  )
  select v_transaction_id, x->>'role', nullif(x->>'source',''), x->>'destination', x->>'asset',
         (x->>'amountAtomic')::numeric, nullif(x->>'tokenMint',''), nullif(x->>'tokenProgram',''),
...
exception
  when unique_violation then return jsonb_build_object('ok', false, 'reason', 'SIGNATURE_ALREADY_BOUND');
end;
$$;
