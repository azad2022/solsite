...
-- The production database already applied this migration as version 20260907214733.
...
create policy pay_merchants_select_member on public.pay_merchants for select to authenticated using (public.pay_has_merchant_access(id));

drop policy if exists pay_merchant_members_select_member on public.pay_merchant_members;
create policy pay_merchant_members_select_member on public.pay_merchant_members for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_merchant_wallets_select_member on public.pay_merchant_wallets;
create policy pay_merchant_wallets_select_member on public.pay_merchant_wallets for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_payment_intents_select_member on public.pay_payment_intents;
create policy pay_payment_intents_select_member on public.pay_payment_intents for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_payment_transactions_select_member on public.pay_payment_transactions;
create policy pay_payment_transactions_select_member on public.pay_payment_transactions for select to authenticated using (exists (select 1 from public.pay_payment_intents p where p.id = payment_id and public.pay_has_merchant_access(p.merchant_id)));

drop policy if exists pay_payment_transfers_select_member on public.pay_payment_transfers;
create policy pay_payment_transfers_select_member on public.pay_payment_transfers for select to authenticated using (exists (select 1 from public.pay_payment_transactions t join public.pay_payment_intents p on p.id = t.payment_id where t.id = payment_transaction_id and public.pay_has_merchant_access(p.merchant_id)));

drop policy if exists pay_payment_events_select_member on public.pay_payment_events;
create policy pay_payment_events_select_member on public.pay_payment_events for select to authenticated using (exists (select 1 from public.pay_payment_intents p where p.id = payment_id and public.pay_has_merchant_access(p.merchant_id)));

drop policy if exists pay_payment_links_select_member on public.pay_payment_links;
create policy pay_payment_links_select_member on public.pay_payment_links for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_invoices_select_member on public.pay_invoices;
create policy pay_invoices_select_member on public.pay_invoices for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_merchant_ledger_select_member on public.pay_merchant_ledger;
create policy pay_merchant_ledger_select_member on public.pay_merchant_ledger for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_gas_accounts_select_member on public.pay_gas_accounts;
create policy pay_gas_accounts_select_member on public.pay_gas_accounts for select to authenticated using (public.pay_has_merchant_access(merchant_id));

drop policy if exists pay_gas_ledger_select_member on public.pay_gas_ledger;
create policy pay_gas_ledger_select_member on public.pay_gas_ledger for select to authenticated using (exists (select 1 from public.pay_gas_accounts g where g.id = gas_account_id and public.pay_has_merchant_access(g.merchant_id)));

drop policy if exists pay_affiliates_select_owner on public.pay_affiliates;
create policy pay_affiliates_select_owner on public.pay_affiliates for select to authenticated using (public.pay_has_affiliate_access(id));

drop policy if exists pay_referrals_select_participant on public.pay_referrals;
create policy pay_referrals_select_participant on public.pay_referrals for select to authenticated using (public.pay_has_merchant_access(merchant_id) or public.pay_has_affiliate_access(affiliate_id));

drop policy if exists pay_commissions_select_participant on public.pay_commissions;
create policy pay_commissions_select_participant on public.pay_commissions for select to authenticated using (exists (select 1 from public.pay_referrals r where r.id = referral_id and (public.pay_has_merchant_access(r.merchant_id) or public.pay_has_affiliate_access(r.affiliate_id))));
