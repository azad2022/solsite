-- Fix-up for the HTTP adapter RPC. The first draft was applied only to the
-- disposable test project and is superseded by this migration before any
-- production adoption.
...
begin
  ...
  case p_operation
    ...
    when 'update' then
      ...
      v_sql:=format('update %s as t set %s from jsonb_populate_record(null::%s,$1) as s where %s returning to_jsonb(t)',v_table,v_set,v_type,v_where); execute v_sql into v_result using p_data; return v_result;
    ...
    when 'delete' then
      v_sql:=format('delete from %s as t where %s returning to_jsonb(t)',v_table,v_where); execute v_sql into v_result; return v_result;
    ...
    when 'increment_one' then
      ...
      v_sql:=format('update %s as t set %s from jsonb_populate_record(null::%s,$1) as s where %s returning to_jsonb(t)',v_table,v_set,v_type,v_where); execute v_sql into v_result using p_set; return v_result;
