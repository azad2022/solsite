-- SolMint Pay: rotate the merchant receiving wallet atomically under the
-- existing one-active-verified-receiving-wallet invariant.
--
-- The previous order promoted the new wallet before deactivating the prior
-- active verified wallet. The unique partial index therefore raised
-- unique_violation during a legitimate wallet rotation and was incorrectly
-- mapped to SIGNATURE_ALREADY_USED. Keep the full operation transactional, but
-- release the existing unique slot before acquiring it for the new wallet.
...
begin
  ...
  -- Release the existing unique slot before promoting the new receiving wallet.
  -- Both updates are part of the same transaction, so callers never observe
  -- the intermediate state; concurrent consumers still serialize on the
  -- challenge and merchant row locks above.
