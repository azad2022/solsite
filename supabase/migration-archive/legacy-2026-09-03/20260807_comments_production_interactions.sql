...
declare
  ...
  v_dislikes integer;
begin
  if p_vote not in (-1, 1) then raise exception 'invalid_vote'; end if;
  ...
  if v_previous is null then
    ...
    delete from public.comment_votes where comment_votes.comment_id = p_comment_id and comment_votes.user_id = p_user_id;
  else
  ...
  update public.comments set like_count = v_likes, dislike_count = v_dislikes
  where id = p_comment_id and approved = true;

  return query select p_comment_id, coalesce(v_likes, 0), coalesce(v_dislikes, 0),
