...
-- The wrappers must be the only executable lifecycle surface for server_role.
DO $$
begin
...
select public.pay_create_api_key(
  'user-a',
  '00000000-0000-0000-0000-000000000001',
  'production',
  'sk_pay_test01',
  repeat('a',64),
  array['payment.create']::text[],
  null,
  'create-1',
  repeat('b',64)
...
declare v jsonb;
begin
  v := :'create_result'::jsonb;
  if v->>'state' <> 'created' then raise exception 'create failed: %',v; end if;
  if exists (select 1 from public.pay_idempotency_keys where (response_body::text like '%sk_pay_plaintext_secret%')) then raise exception 'plaintext secret was persisted'; end if;
...
select public.pay_revoke_api_key('user-a','00000000-0000-0000-0000-000000000001',(select id from public.pay_api_keys limit 1)) as revoke_result \gset
DO $$ begin
  if :'revoke_result'::jsonb->>'state' <> 'revoked' then raise exception 'revoke failed'; end if;
  if (select revoked_at is null from public.pay_api_keys limit 1) then raise exception 'key was not revoked'; end if;
end $$;

insert into public.pay_api_keys(merchant_id,name,key_prefix,key_hash,scopes) values
('00000000-0000-0000-0000-000000000001','rotate-me','sk_pay_old','1'.repeat(64),array['payment.create']::text[]);
select public.pay_rotate_api_key(
  'user-a','00000000-0000-0000-0000-000000000001',
  (select id from public.pay_api_keys where key_prefix='sk_pay_old'),
  'rotated','sk_pay_new',repeat('2',64),array['payment.create']::text[],null,'rotate-1',repeat('3',64)
) as rotate_result \gset
DO $$
declare v_old_revoked boolean; v jsonb;
begin
  v := :'rotate_result'::jsonb;
  if v->>'state' <> 'created' then raise exception 'rotate failed: %',v; end if;
  select revoked_at is not null into v_old_revoked from public.pay_api_keys where key_prefix='sk_pay_old';
  if not v_old_revoked then raise exception 'old key remained active after rotate'; end if;
