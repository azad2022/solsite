...
if (!source.includes('const [commentCounts, setCommentCounts]')) {
  ...
  if (source === categoryBefore) throw new Error('Comment count effect insertion failed.');
}
...
if (addStart < 0 || addEnd <= addStart) throw new Error('Legacy BlogHub comment submit handler not found.');
source = source.slice(0, addStart) + source.slice(addEnd);

const countBefore = source;
source = source.replace(/\{art\.comments\.length\} دیدگاه/g, "{commentCounts[String(art.id)] ?? 0} دیدگاه");
if (source === countBefore) throw new Error('Article comment count binding not found.');
...
if (modalStart < 0 || modalEnd <= modalStart) throw new Error('Legacy BlogHub article comments block not found.');
...
console.log('production-comments-unification: BlogHub is now guarded by the canonical CommentsSection and live production counts.');
