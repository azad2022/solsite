...
function patch(path, transform, description, { allowNoop = false } = {}) {
  ...
  if (result === source) {
    ...
    throw new Error(`[category-media] No changes made to ${path}: ${description}`);
  }
...
patch('functions/api/article-categories.ts', source => {
  if (source.includes('default_media_asset_id?: string | null') && source.includes('default_media_url?: string | null') && source.includes('default_media_asset_id: category.default_media_asset_id || null')) return source;
  let out = source;
  out = out.replace(
  ...
  return out;
}, 'persist category default media fields', { allowNoop: true });
...
patch('src/components/ArticleCategoryManager.tsx', source => {
  ...
  if (!out.includes("import { getAllMediaAssets } from '../utils/mediaService';")) {
    ...
    if (!out.includes(anchor)) throw new Error('[category-media] category manager import anchor not found');
    out = out.replace(anchor, anchor + "import { getAllMediaAssets } from '../utils/mediaService';\n");
  }
  out = out.replace("  updated_at: string;\n}", "  updated_at: string;\n  default_media_asset_id?: string | null;\n  default_media_url?: string | null;\n}");
  out = out.replace("interface FormState { name: string; slug: string; description: string; seo_title: string; seo_description: string; parent_id: string; sort_order: string; is_active: boolean; }", "interface FormState { name: string; slug: string; description: string; seo_title: string; seo_description: string; parent_id: string; sort_order: string; is_active: boolean; default_media_asset_id: string; default_media_url: string; }");
  out = out.replace("const emptyForm = (): FormState => ({ name: '', slug: '', description: '', seo_title: '', seo_description: '', parent_id: '', sort_order: '100', is_active: true });", "const emptyForm = (): FormState => ({ name: '', slug: '', description: '', seo_title: '', seo_description: '', parent_id: '', sort_order: '100', is_active: true, default_media_asset_id: '', default_media_url: '' });");
  out = out.replace("  const [notice, setNotice] = useState<{ success?: boolean; message: string } | null>(null);", "  const [notice, setNotice] = useState<{ success?: boolean; message: string } | null>(null);\n  const [mediaAssets, setMediaAssets] = useState<Array<{ id: string; filename: string; publicUrl: string; title?: string; altText?: string }>>([]);\n  const [mediaLoading, setMediaLoading] = useState(false);");
  out = out.replace("  useEffect(() => { void load(); }, []);", "  useEffect(() => { void load(); void loadMediaAssets(); }, []);\n\n  const loadMediaAssets = async () => {\n    setMediaLoading(true);\n    try {\n      const assets = await getAllMediaAssets();\n      setMediaAssets(assets as any);\n    } catch {} finally { setMediaLoading(false); }\n  };");
  out = out.replace("const payload = { ...form, name: form.name.trim(), slug: form.slug.trim(), parent_id: form.parent_id || null, sort_order: Number(form.sort_order) || 100 };", "const payload = { ...form, name: form.name.trim(), slug: form.slug.trim(), parent_id: form.parent_id || null, sort_order: Number(form.sort_order) || 100, default_media_asset_id: form.default_media_asset_id || null, default_media_url: form.default_media_url || null };");
  out = out.replace("const edit = (category: ArticleCategory) => setForm({ name: category.name, slug: category.slug, description: category.description || '', seo_title: category.seo_title || '', seo_description: category.seo_description || '', parent_id: category.parent_id || '', sort_order: String(category.sort_order), is_active: category.is_active });", "const edit = (category: ArticleCategory) => setForm({ name: category.name, slug: category.slug, description: category.description || '', seo_title: category.seo_title || '', seo_description: category.seo_description || '', parent_id: category.parent_id || '', sort_order: String(category.sort_order), is_active: category.is_active, default_media_asset_id: category.default_media_asset_id || '', default_media_url: category.default_media_url || '' });");
