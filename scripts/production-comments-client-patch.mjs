...
const targetStart = source.indexOf('export async function deleteCommentApi');
const targetSlice = targetStart >= 0 ? source.slice(targetStart, targetStart + 700) : '';
if (targetSlice.includes('headers: getAuthHeaders()')) {
  console.log('Production comments client patch already applied.');
  process.exit(0);
}
...
if (source === original) throw new Error('deleteCommentApi patch target was not found');
