...
ledger_versions() {
  awk '/^[0-9]{14} / {print $1}' "$LEDGER_FILE"
}
...
require_guard() {
  ...
  [[ "${SUPABASE_ACCESS_TOKEN:-}" != "" ]] || { echo "Refusing: SUPABASE_ACCESS_TOKEN is required" >&2; exit 1; }
...
case "$action" in
  plan)
    ...
    echo "Forward: repair each historical version -> reverted, then baseline -> applied."
    echo "Rollback: baseline -> reverted, then historical versions -> applied in original order."
  ...
  apply)
    ...
    rollback_on_error() {
      set +e
      for ((i=completed-1; i>=0; i--)); do supabase migration repair --project-ref "$SUPABASE_PROJECT_REF" --status applied "${versions[$i]}" >/dev/null; done
      rm -f "$STATE_FILE"
      exit 1
    }
    trap rollback_on_error ERR
    for version in "${versions[@]}"; do
      supabase migration repair --project-ref "$SUPABASE_PROJECT_REF" --status reverted "$version"
  ...
  rollback)
    ...
    applied=0
    rollback_forward_on_error() {
      set +e
      for ((i=applied-1; i>=0; i--)); do supabase migration repair --project-ref "$SUPABASE_PROJECT_REF" --status reverted "${versions[$i]}" >/dev/null; done
      exit 1
    }
    trap rollback_forward_on_error ERR
    for version in "${versions[@]}"; do
      supabase migration repair --project-ref "$SUPABASE_PROJECT_REF" --status applied "$version"
      applied=$((applied+1))
    done
    trap - ERR
    rm -f "$STATE_FILE"
