...
async function issueChallenge(cookie: string, walletAddress: string): Promise<{ response: Response; body: Record<string, unknown> }> {
  const response = await request(`/api/pay/v1/merchants/${encodeURIComponent(MERCHANT_ID)}/wallet-challenges`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
test('authenticated Wallet Ownership Lifecycle is isolated and server-authoritative', async () => {
  ...
  const noSession = await request(`/api/pay/v1/merchants/${encodeURIComponent(MERCHANT_ID)}/wallet-challenges`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const badOrigin = await request(`/api/pay/v1/merchants/${encodeURIComponent(MERCHANT_ID)}/wallet-challenges`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ walletAddress: createWalletSigner().address }),
