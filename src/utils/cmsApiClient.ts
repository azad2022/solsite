...
function getAuthHeaders(): Record<string, string> {
  const passcode = localStorage.getItem('solmint_admin_passcode') || localStorage.getItem('solmint_passcode') || '';
  return {
...
export async function deleteCommentApi(commentId: string): Promise<boolean> {
  try {
    const res = await fetch('/api/comments/delete', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ commentId }) });
    return res.ok;
  } catch (err) {
...
export async function saveArticleToApi(article: Article): Promise<boolean> {
  try {
    const res = await fetch('/api/articles', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify(article) });
    return res.ok;
  } catch (err) {
