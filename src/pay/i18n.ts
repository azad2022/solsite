...
const messages = {
  'fa-IR': {
    ...
    checkout: 'پرداخت', checkoutSecure: 'اتصال امن', backToPay: 'بازگشت به Pay', checkoutWaitingTitle: 'در انتظار Payment Intent معتبر', checkoutWaitingDescription: 'جزئیات پرداخت فقط از snapshot معتبر سرویس پرداخت نمایش داده می‌شود؛ این صفحه هیچ مبلغ، مقصد یا کارمزد را خودش محاسبه نمی‌کند.', paymentState: 'وضعیت پرداخت', awaitingIntent: 'در انتظار Intent', verification: 'تأیید', verificationPending: 'در انتظار سرویس تأیید', expiration: 'انقضا', awaitingSnapshot: 'هنوز دریافت نشده', checkoutSnapshot: 'مرجع پرداخت', checkoutSnapshotDescription: 'برای شروع پرداخت، یک Payment Intent معتبر باید از Backend دریافت شود. ارسال تراکنش به‌تنهایی موفقیت پرداخت محسوب نمی‌شود.', checkoutIntentMissing: 'شناسه Intent در مسیر وجود ندارد.',
  },
  'en-US': {
...
export function sectionLabel(locale: PayLocale, section: PaySection): string {
  const mapping: Record<PaySection, MessageKey> = {
    overview: 'overview', transactions: 'transactions', merchants: 'merchants', customers: 'customers', invoices: 'invoices', referrals: 'referrals', reports: 'reports', tickets: 'tickets', developer: 'developer', security: 'security',
  };
