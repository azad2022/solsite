...
import { AlertCircle, ChevronLeft, ChevronRight, CircleCheck, Clock3, Loader2, MessageSquare, Plus, RefreshCw, Send, ShieldCheck } from 'lucide-react';
...
function isForbidden(error: unknown): boolean { return error instanceof PayHttpError && (error.status === 403 || error.status === 401); }
...
export default function PayTicketCenter({ locale, sessionUser, merchantId }: Props): React.ReactElement {
  ...
  async function loadTickets(selectFirst=true) { setLoading(true); setError(null); try { const rows=await listPayTickets({merchantId:isAdmin?undefined:merchantId}); setTickets(rows); if (selectFirst && rows[0]) setSelectedId(current=>current && rows.some(item=>item.id===current)?current:rows[0].id); else if (!rows.length) setSelectedId(null); } catch (e) { setError(isForbidden(e)?'forbidden':e instanceof PayHttpError&&e.status===401?'unauthorized':'error'); } finally { setLoading(false); } }
  async function loadDetail(id:string) { setDetailLoading(true); setDetailError(null); try { setDetail(await getPayTicket(id)); } catch(e) { setDetailError(isForbidden(e)?'forbidden':e instanceof PayHttpError&&e.status===401?'unauthorized':'error'); } finally { setDetailLoading(false); } }
  ...
  async function handleCreate(event:React.FormEvent) { event.preventDefault(); if(!merchantId || !subject.trim() || !message.trim() || busy) return; setBusy('create'); setError(null); try { const created=await createPayTicket({merchantId,subject,message,priority}); setSubject(''); setMessage(''); setPriority('normal'); await loadTickets(false); setSelectedId(created.id); } catch(e) { setError(isForbidden(e)?'forbidden':e instanceof PayHttpError&&e.status===401?'unauthorized':'error'); } finally { setBusy(null); } }
  async function handleReply(event:React.FormEvent) { event.preventDefault(); if(!selectedId || !reply.trim() || busy) return; setBusy('reply'); setDetailError(null); try { await replyToPayTicket(selectedId,reply); setReply(''); await Promise.all([loadTickets(false),loadDetail(selectedId)]); } catch(e) { setDetailError(isForbidden(e)?'forbidden':e instanceof PayHttpError&&e.status===401?'unauthorized':'error'); } finally { setBusy(null); } }
  async function handleStatus(status:PayTicketStatus) { if(!selectedId || busy || !isAdmin) return; setBusy('status'); try { const updated=await updatePayTicketStatus(selectedId,status); setTickets(current=>current.map(item=>item.id===updated.id?updated:item)); setDetail(current=>current?{...current,ticket:updated}:current); } catch(e) { setDetailError(isForbidden(e)?'forbidden':e instanceof PayHttpError&&e.status===401?'unauthorized':'error'); } finally { setBusy(null); } }

  return <section className="pay-ticket-center" aria-label={ticketT(locale,'tickets')}>
    <div className="pay-ticket-heading"><div><div className="pay-panel-kicker">{isAdmin?<>{ticketT(locale,'adminInbox')}</>:ticketT(locale,'support')}</div><h2>{ticketT(locale,'title')}</h2><p>{ticketT(locale,'subtitle')}</p></div><button type="button" className="pay-icon-button" onClick={()=>void loadTickets(false)} aria-label={ticketT(locale,'retry')}><RefreshCw size={17}/></button></div>
    ...
    <div className="pay-ticket-layout">
      ...
      <div className="pay-ticket-detail-pane">
        {!selected ? <div className="pay-ticket-detail-empty"><MessageSquare size={25}/><p>{isAdmin?ticketT(locale,'selectTicket'):ticketT(locale,'selectTicket')}</p></div> : detailLoading && !detail ? <div className="pay-ticket-loading"><Loader2 className="spin" size={20}/>{ticketT(locale,'loading')}</div> : <div className="pay-ticket-thread"><div className="pay-ticket-thread-header"><div><span className="pay-panel-kicker">{ticketT(locale,'details')}</span><h3>{selected.subject}</h3><p>{ticketT(locale,'status')}: {translateStatus(locale,selected.status)} · {priorityLabel(locale,selected.priority)}</p></div>{isAdmin?<select value={selected.status} disabled={busy==='status'} onChange={e=>void handleStatus(e.target.value as PayTicketStatus)} aria-label={ticketT(locale,'updateStatus')}>{STATUSES.map(value=><option key={value} value={value}>{translateStatus(locale,value)}</option>)}</select>:null}</div>
