...
const errorMessage = (response: Response, payload: AuthResponse) => {
  ...
  return payload.message || payload.error || 'درخواست احراز هویت انجام نشد. دوباره تلاش کنید.';
};
      ...
      <div className="relative z-10 max-h-[92vh] w-full max-w-md overflow-y-auto rounded-3xl border border-white/10 bg-[#0b0b13] p-5 text-slate-100 shadow-2xl shadow-black/50 sm:p-7" dir="rtl">
        ...
        <div className="mb-6 pr-1">
          <div className="mb-3 flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl border border-[#14F195]/20 bg-[#14F195]/10 text-[#14F195]"><ShieldCheck className="h-5 w-5" /></div>
            <div>
              <h2 className="text-xl font-black text-white">حساب سولمینت</h2>
              <p className="mt-0.5 text-xs text-slate-500">احراز هویت امن با نشست HttpOnly</p>
            </div>
          </div>
        </div>

        {mode !== 'forgot' && (
          <button type="button" onClick={startGoogleOAuth} disabled={busy} className="mb-4 flex w-full items-center justify-center gap-2.5 rounded-2xl border border-white/10 bg-white px-4 py-3 text-sm font-black text-slate-900 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50">
        ...
        {mode === 'login' && (
          <div className="mb-4 grid grid-cols-2 rounded-2xl border border-white/10 bg-white/[0.03] p-1">
            <button type="button" onClick={() => setLoginMethod('email')} className={`rounded-xl px-3 py-2 text-xs font-bold transition ${loginMethod === 'email' ? 'bg-[#9945FF]/20 text-white' : 'text-slate-500 hover:text-slate-300'}`}>ایمیل</button>
            <button type="button" onClick={() => setLoginMethod('username')} className={`rounded-xl px-3 py-2 text-xs font-bold transition ${loginMethod === 'username' ? 'bg-[#9945FF]/20 text-white' : 'text-slate-500 hover:text-slate-300'}`}>نام کاربری</button>
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-3.5">
          {mode === 'register' && <>
            <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-400">نام نمایشی</span><div className="relative"><User className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-600" /><input value={name} onChange={e => setName(e.target.value)} autoComplete="name" className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3 pr-10 pl-3 text-sm outline-none transition focus:border-[#14F195]/40 focus:bg-white/[0.06]" placeholder="نام شما" /></div></label>
            <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-400">نام کاربری</span><div className="relative"><UserPlus className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-600" /><input value={username} onChange={e => setUsername(e.target.value)} autoComplete="username" className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3 pr-10 pl-3 text-sm outline-none transition focus:border-[#14F195]/40 focus:bg-white/[0.06]" placeholder="مثلاً solmint_user" /></div></label>
          </>}

          {mode !== 'login' || loginMethod === 'email' ? <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-400">ایمیل</span><div className="relative"><Mail className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-600" /><input value={email} onChange={e => setEmail(e.target.value)} type="email" autoComplete={mode === 'login' ? 'email' : 'email'} className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3 pr-10 pl-3 text-sm outline-none transition focus:border-[#14F195]/40 focus:bg-white/[0.06]" placeholder="you@example.com" /></div></label> : null}
          ...
          {mode !== 'forgot' && <>
            <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-400">رمز عبور</span><div className="relative"><LockKeyhole className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-600" /><input value={password} onChange={e => setPassword(e.target.value)} type={showPassword ? 'text' : 'password'} autoComplete={mode === 'login' ? 'current-password' : 'new-password'} className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-3 pr-10 pl-11 text-sm outline-none transition focus:border-[#14F195]/40 focus:bg-white/[0.06]" placeholder="••••••••" /><button type="button" aria-label={showPassword ? 'پنهان کردن رمز' : 'نمایش رمز'} onClick={() => setShowPassword(value => !value)} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-slate-300"><EyeOff className={`h-4 w-4 ${showPassword ? 'hidden' : ''}`} /><Eye className={`h-4 w-4 ${showPassword ? '' : 'hidden'}`} /></button></div></label>
            {mode === 'register' && <label className="block"><span className="mb-1.5 block text-xs font-bold text-slate-400">تکرار رمز عبور</span><input value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} type={showPassword ? 'text' : 'password'} autoComplete="new-password" className="w-full rounded-2xl border border-white/10 bg-white/[0.04] px-3 py-3 text-sm outline-none transition focus:border-[#14F195]/40 focus:bg-white/[0.06]" placeholder="••••••••" /></label>}
          </>}
          ...
          <button type="submit" disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#9945FF] px-4 py-3.5 text-sm font-black text-white shadow-lg shadow-[#9945FF]/15 transition hover:bg-[#a558ff] disabled:cursor-not-allowed disabled:opacity-50">
            {mode === 'login' ? <KeyRound className="h-4 w-4" /> : mode === 'register' ? <UserPlus className="h-4 w-4" /> : <Mail className="h-4 w-4" />}
            {busy ? 'در حال پردازش…' : mode === 'login' ? 'ورود امن' : mode === 'register' ? 'ایجاد حساب' : 'ارسال لینک بازیابی'}
          </button>
        </form>

        <div className="mt-5 flex items-center justify-between gap-2 text-xs">
          {mode === 'login' && <>
            <button type="button" onClick={() => switchMode('forgot')} className="font-bold text-slate-500 hover:text-slate-200">رمز عبور را فراموش کرده‌اید؟</button>
            <button type="button" onClick={() => switchMode('register')} className="font-bold text-[#14F195] hover:text-white">ثبت‌نام</button>
          </>}
          {mode === 'register' && <button type="button" onClick={() => switchMode('login')} className="font-bold text-[#14F195] hover:text-white">بازگشت به ورود</button>}
          {mode === 'forgot' && <button type="button" onClick={() => switchMode('login')} className="font-bold text-[#14F195] hover:text-white">بازگشت به ورود</button>}
        </div>

        <p className="mt-5 flex items-start gap-2 border-t border-white/5 pt-4 text-[11px] leading-5 text-slate-600">
          <LockKeyhole className="mt-0.5 h-3.5 w-3.5 shrink-0" />
          نشست احراز هویت در کوکی امن HttpOnly نگهداری می‌شود و این رابط هیچ توکن یا session را در localStorage ذخیره نمی‌کند.
        </p>
      </div>
    </div>
  );
