...
import { Send, Menu, X, BookOpen, User, LogOut, ShieldCheck, Smartphone } from 'lucide-react';
  ...
  return (
    <header className="relative w-full z-40 bg-[#05050a]/90 backdrop-blur-xl border-b border-white/[0.08]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-0.5 bg-white/[0.03] p-1 rounded-full border border-white/10 whitespace-nowrap">
          <a href="/" onClick={e => { e.preventDefault(); handleNav('/'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer text-inherit decoration-none ${currentPath === '/' ? 'bg-white/10 text-white font-bold' : 'text-slate-300 hover:text-white'}`}>صفحه اصلی</a>
          <a href="/solana-wallet" onClick={e => { e.preventDefault(); handleNav('/solana-wallet'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer text-inherit decoration-none ${currentPath === '/solana-wallet' ? 'bg-[#9945FF]/20 text-[#14F195] font-bold border border-[#9945FF]/40' : 'text-slate-300 hover:text-white'}`}>کیف پول سولانا</a>
          <a href="/solana-price" onClick={e => { e.preventDefault(); handleNav('/solana-price'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer text-inherit decoration-none ${currentPath === '/solana-price' ? 'bg-[#14F195]/15 text-[#14F195] border border-[#14F195]/30 shadow-sm shadow-[#14F195]/10' : 'text-slate-300 hover:text-white'}`}>قیمت لحظه‌ای سولانا</a>
          <a href="/solana-token" onClick={e => { e.preventDefault(); handleNav('/solana-token'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer text-inherit decoration-none ${currentPath === '/solana-token' ? 'bg-cyan-500/20 text-cyan-300 font-bold border border-cyan-500/30' : 'text-slate-300 hover:text-white'}`}>ساخت توکن</a>
          <a href="/solana-meme-coin" onClick={e => { e.preventDefault(); handleNav('/solana-meme-coin'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer text-inherit decoration-none ${currentPath === '/solana-meme-coin' ? 'bg-amber-500/20 text-amber-300 font-bold border border-amber-500/30' : 'text-slate-300 hover:text-white'}`}>میم کوین</a>
          <a href="/security" onClick={e => { e.preventDefault(); handleNav('/security'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer text-inherit decoration-none ${currentPath === '/security' ? 'bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/30' : 'text-slate-300 hover:text-white'}`}>امنیت</a>
          <a href="/faq" onClick={e => { e.preventDefault(); handleNav('/faq'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-semibold transition-colors cursor-pointer text-inherit decoration-none ${currentPath === '/faq' ? 'bg-slate-800 text-white font-bold' : 'text-slate-300 hover:text-white'}`}>سوالات متداول</a>
          <a href="/blog" onClick={e => { e.preventDefault(); handleNav('/blog'); }} className={`shrink-0 whitespace-nowrap px-2.5 py-1.5 rounded-full text-xs font-bold transition-all cursor-pointer flex items-center gap-1.5 text-inherit decoration-none ${currentPath === '/blog' || currentPath.startsWith('/article/') ? 'bg-gradient-to-r from-[#9945FF] to-[#14F195] text-black shadow-md' : 'text-slate-300 hover:text-white'}`}><BookOpen className="w-3.5 h-3.5 shrink-0" /><span className="whitespace-nowrap">وبلاگ</span></a>
        </nav>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#08080f]/95 border-b border-white/10 px-4 py-5 space-y-2 backdrop-blur-2xl">
          ...
          <button onClick={() => handleNav('/solana-price')} className={`w-full text-right px-4 py-2.5 rounded-xl text-xs font-bold ${currentPath === '/solana-price' ? 'bg-[#14F195]/15 text-[#14F195] border border-[#14F195]/30' : 'text-slate-300 bg-white/5'}`}>قیمت لحظه‌ای سولانا</button>
