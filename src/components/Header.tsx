...
export const Header: React.FC<HeaderProps> = ({ currentPath, onNavigate, openAdminModal, currentUser, onLogout }) => {
  ...
  const { user: sessionUser } = useApplicationSession();
  ...
  const canManageShowcase = effectiveUser?.role === 'admin' || effectiveUser?.role === 'superadmin';
        ...
        <div className="overflow-visible rounded-2xl border border-white/[0.10] bg-[#05050a]/95 shadow-[0_14px_50px_rgba(0,0,0,0.28)] backdrop-blur-xl">
          <div className="flex h-16 items-center justify-between gap-3 px-3 sm:px-5">
            ...
            <div className="hidden shrink-0 items-center gap-2.5 sm:flex">
              {effectiveUser ? <div className="flex items-center gap-2 rounded-xl border border-slate-700/80 bg-slate-900 px-3 py-1.5 text-xs text-slate-200">
                ...
                <button onClick={openAdminModal} className="flex items-center gap-1.5 font-bold transition-colors hover:text-[#14F195]">{canManageShowcase ? <ShieldCheck className="h-4 w-4 text-emerald-400" /> : <User className="h-4 w-4 text-sky-400" />}<span>{effectiveUser.fullName}</span>{canManageShowcase && <span className="rounded bg-emerald-500/20 px-1.5 py-0.5 font-mono text-[10px] text-emerald-300">مدیر</span>}</button>
                <span className="text-slate-600">|</span><button onClick={() => void handleLogout()} title="خروج از حساب" aria-label="خروج از حساب" className="p-0.5 text-slate-400 transition-colors hover:text-rose-400"><LogOut className="h-3.5 w-3.5" /></button>
          ...
          {mobileMenuOpen && <div className="border-t border-white/10 bg-[#08080f]/98 px-4 py-5 backdrop-blur-2xl lg:hidden">
            <div className="space-y-2">
              ...
              {effectiveUser && canManageShowcase && <button onClick={() => handleNav('/showcase-admin')} className="flex w-full items-center gap-2 rounded-xl border border-[#14F195]/25 bg-[#14F195]/10 px-4 py-2.5 text-right text-xs font-bold text-[#14F195]"><Smartphone className="h-4 w-4" />مدیریت نمایش اپلیکیشن</button>}
              {effectiveUser ? <div className="mt-2 flex items-center justify-between gap-2 rounded-xl border border-slate-700/80 bg-slate-900 px-4 py-3"><button onClick={openAdminModal} className="flex items-center gap-2 text-xs font-bold text-slate-200">{canManageShowcase ? <ShieldCheck className="h-4 w-4 text-emerald-400" /> : <User className="h-4 w-4 text-sky-400" />}<span>{effectiveUser.fullName}</span></button><button onClick={() => void handleLogout()} title="خروج از حساب" aria-label="خروج از حساب" className="p-1 text-slate-400 hover:text-rose-400"><LogOut className="h-4 w-4" /></button></div> : <button onClick={openAdminModal} className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-bold text-slate-200 hover:bg-white/10"><User className="h-4 w-4 text-[#14F195]" />ورود / ثبت‌نام</button>}
            </div>
          </div>}
        </div>
      </div>
