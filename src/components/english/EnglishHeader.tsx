  ...
  return <header dir="ltr" className="relative w-full z-40 bg-[#05050a]/90 backdrop-blur-xl border-b border-white/[0.08]">
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-3">
      ...
      <nav className="hidden xl:flex items-center gap-0.5 bg-white/[0.03] p-1 rounded-full border border-white/10 whitespace-nowrap">
        {primaryLinks.map(([path, label]) => <a key={path} href={path} onClick={e => { e.preventDefault(); go(path); }} className={linkClass(currentPath === path)}>{label}</a>)}
        <div className="relative" onMouseEnter={() => setToolsOpen(true)} onMouseLeave={() => setToolsOpen(false)}>
          <button type="button" onClick={() => go('/en/tools')} className={`${linkClass(currentPath.startsWith('/en/tools'))} inline-flex items-center gap-1`}><Wrench className="w-3.5 h-3.5" />Tools<ChevronDown className={`w-3 h-3 ${toolsOpen ? 'rotate-180' : ''}`} /></button>
