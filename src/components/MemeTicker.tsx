...
const MarketItem: React.FC<{ item: MemeTickerItem }> = ({ item }) => {
  ...
  const [useLocal, setUseLocal] = useState(!item.logoUrl);
  const remoteLogo = item.logoUrl || '';
  const localLogo = LOCAL_LOGOS[symbol] || '';
  const logoSrc = useLocal ? localLogo : remoteLogo;
  ...
  const content = (
    <>
      <img
        ...
        loading="eager"
        decoding="async"
        onError={() => { if (remoteLogo && localLogo && !useLocal) setUseLocal(true); }}
      />
...
export const MemeTicker: React.FC = () => {
  ...
  const items = useMemo(() => (feed.items || []).filter(item => item.enabled && item.priceUsd !== null && (item.logoUrl || LOCAL_LOGOS[item.symbol.toUpperCase()])).sort((a, b) => a.order - b.order), [feed.items]);
