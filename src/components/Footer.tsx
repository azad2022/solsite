...
function toTags(value: unknown): string[] {
  ...
  if (typeof value === 'string') {
    try { const parsed = JSON.parse(value); if (Array.isArray(parsed)) return parsed.map(item => String(item)).filter(Boolean); } catch {}
  ...
  return (
    <section className="w-full" dir="rtl" aria-labelledby="footer-related-articles-title">
      <div className="rounded-3xl border border-slate-800/80 bg-slate-950/70 p-5 sm:p-7">
        ...
        <nav className="mt-4" aria-label="مقالات مرتبط"><ul className="divide-y divide-slate-800/80">
          {related.map(article => <li key={article.id || article.slug}><a href={`/article/${encodeURIComponent(String(article.slug))}`} className="block py-3 text-sm font-bold leading-7 text-slate-200 transition hover:text-[#14F195] sm:text-base">{article.title}</a></li>)}
        </ul></nav>
      </div>
    </section>
  );
  ...
  return (
    <footer className="bg-[#05050a] border-t border-white/[0.08] pt-16 pb-10 text-slate-300 text-xs sm:text-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8"><div className="space-y-3 text-center md:text-right">
