...
export interface Article {
  ...
  title: string;
  slug: string;
  /** Language metadata is optional so existing production content remains compatible during migration. */
  language?: 'fa' | 'en';
...
export interface Testimonial { id: string; name: string; role: string; comment: string; avatar: string; stars: number; createdAt: string; createdAtJalali?: string; createdAtGregorian?: string; approved?: boolean; }

export interface DownloadLinks { apkUrl: string; telegramUrl: string; googlePlayUrl?: string; webAppUrl?: string; apkVersion?: string; downloadNotice?: string; }

export const DEFAULT_DOWNLOAD_LINKS: DownloadLinks = {
  apkUrl: 'https://t.me/solmintchannel', telegramUrl: 'https://t.me/solmintchannel', googlePlayUrl: 'https://play.google.com/store/apps', webAppUrl: 'https://app.solmint.ir', apkVersion: 'v2.4.0', downloadNotice: 'تست شده با Play Protect گوگل و بدون نیاز به دسترسی‌های مشکوک'
};

export interface DeepSeekAiSettings {
  apiKey: string; apiKeyConfigured?: boolean; apiBaseUrl: string; model: string; systemPrompt: string; requireCoverImage?: boolean; targetTopics: string[]; targetKeywords: string[];
  publishSchedule: { enabled: boolean; publishDays: string[]; publishTime: string; publishMode?: 'published' | 'draft'; autoPublishAsDraft: boolean; timezone?: string; intervalHours?: number; };
  mediaConfig: { includeCoverImage: boolean; requireCoverImage?: boolean; imageStyle: 'solana_theme' | 'cyberpunk_crypto' | 'tech_minimal' | '3d_gradient'; includeVideo: boolean; defaultVideoUrl?: string; };
  writingStyle: { tone: 'آموزشی و روان' | 'تخصصی و فنی' | 'خبری و تحلیلی' | 'عامیانه و صمیمی'; targetWordCount: number; includeFaqSection: boolean; includeCallToAction: boolean; };
  autoPublishEnabled?: boolean; publishScheduleHours?: number; lastAutoPublishedAt?: string; lastPublishedSlot?: string; lastExecutionStatus?: 'success' | 'error' | 'running'; lastExecutionMessage?: string;
}

export const DEFAULT_DEEPSEEK_SETTINGS: DeepSeekAiSettings = {
  apiKey: '', apiKeyConfigured: false, apiBaseUrl: 'https://api.deepseek.com/v1', model: 'deepseek-chat',
...
۳. حتماً از کلمات کلیدی تعیین شده در طول متن به طور طبیعی استفاده کنید.
...
۵. قوانین اکید عنوان و محتوا: به هیچ عنوان کلماتی نظیر "مقاله سئو شده"، "آموزش سئو شده"، "سئو شده" یا نام‌های هوش مصنوعی (مانند DeepSeek) را در عنوان مقاله یا متن یا به عنوان نویسنده یا لینک وارد نکنید. فقط عنوان اصلی مقاله درج شود.`,
  ...
  targetTopics: ['آموزش جامع ساخت توکن در شبکه‌ی سولانا بدون کدنویسی','راهنمای ساخت میم کوین با سولمینت و افزودن نقدینگی','بازیابی کارمزد اجاره حساب‌های خالی سولانا (SOL Rent Claim)','آموزش ضرب NFT با استاندارد Metaplex در اپلیکیشن موبایل','بررسی امنیت کیف پول‌های غیرامانی و الگوریتم Ed25519','مقایسه کارمزد و سرعت سولانا با اتریوم و تون‌کوین (TON)','چگونه اولین آردراپ (Airdrop) خود را در بلاکچین سولانا دریافت کنیم؟'],
  targetKeywords: ['سولمینت','کیف پول سولانا','ساخت توکن سولانا','بازیابی اجاره SOL','ساخت میم کوین','کیف پول غیرامانی','ضرب NFT سولانا','برنامه سولمینت'],
  publishSchedule: { enabled: true, publishDays: ['شنبه','یکشنبه','دوشنبه','سه‌شنبه','چهارشنبه','پنج‌شنبه','جمعه'], publishTime: '10:00', publishMode: 'published', autoPublishAsDraft: false, timezone: 'Asia/Tehran', intervalHours: 6 },
  mediaConfig: { includeCoverImage: true, imageStyle: 'solana_theme', includeVideo: false, defaultVideoUrl: 'https://assets.mixkit.co/videos/preview/mixkit-digital-animation-of-screens-and-code-42898-large.mp4' },
  writingStyle: { tone: 'آموزشی و روان', targetWordCount: 1200, includeFaqSection: true, includeCallToAction: true }
};
